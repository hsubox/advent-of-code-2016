...
number = 0
while found < 64:
    hash1 = md5.new(input + str(number)).hexdigest()
    m = re.search(r'([a-z0-9]{1})\1\1', hash1)
    if m != None:
        for i in xrange(number + 1, number + 1000):
            hash2 = md5.new(input + str(i)).hexdigest()
...
# store hashes to speed up calculations as each is used multiple times
hashes = []
def calculateHash(input, number):
    if number >= len(hashes):
        hash = md5.new(input + str(number)).hexdigest()
        for i in range(2016):
...
number = 0
while found < 64:
    hash1 = calculateHash(input, number)
    m = re.search(r'([a-z0-9]{1})\1\1', hash1)
    if m != None:
        for i in xrange(number + 1, number + 1000):
            hash2 = calculateHash(input, i)
            n = re.search(r'(' + m.group(1) + '{1})\\1{4}', hash2)
            if n != None:
                found += 1
                break
