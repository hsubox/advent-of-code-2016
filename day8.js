...
var screen = Array(height).fill('.').map((y) => {
...
function rotateRow(rowNum, rightBy) {
  var copyOfRow = screen[rowNum].slice();
  for (var i = 0; i < width; i++) {
    screen[rowNum][i] = copyOfRow[(width * 2 + i - rightBy) % width];
  }
}

function rotateColumn(colNum, downBy) {
  copyOfCol = [];
  for (var i = 0; i < height; i++) {
    copyOfCol.push(screen[i][colNum]);
  }
  for (var j = 0; j < height; j++) {
    screen[j][colNum] = copyOfCol[(height * 2 + j - downBy) % height];
  }
}
...
input.split('\n').forEach(function(instruction) {
  ...
  switch(words[0]) {
    case 'rect':
      var dimensions = words[1].split('x').map(Number);
      addRect(dimensions[0], dimensions[1]);
      break;
    case 'rotate':
      var values = /=(\d+) by (\d+)/gi.exec(instruction).slice(1, 3).map(Number);
      switch(words[1]) {
        case 'row':
...
console.log(screen.map((a) => a.reduce((a, b) => (b == '#') ? a : a + 1, 0), 0).reduce((a, b) => a + b, 0));
