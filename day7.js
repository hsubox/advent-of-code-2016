...
var ipList = input.split('\n').filter((ip) => {
  return !(/\[\w*(\w{1})(\w{1})\2\1\w*\]/g.test(ip));
}).filter((ip) => {
  return (containedABBA = ip.match(/(\w{1})(\w{1})\2\1/g)) ? containedABBA.some((abba) => abba[0] != abba[1]) : false;
});
console.log(ipList.length);


var ipList = input.split('\n').filter((ip) => {
  var supernets = ip.split(/\[\w*\]/g).join(' ');
  var hypernets = ip.split(/^\w*\[|\]\w*\[|\]\w*$/g).filter(x => x != '').join(' ');
  var ABAs = [];
  var regex = /(\w{1})\w{1}\1/g;
  while(match = regex.exec(hypernets)) {
    ABAs.push(match[0]);
    regex.lastIndex = match.index + 1;
  }
  return ABAs.filter(ABA => ABA[0] != ABA[1]).some(ABA => {
    var BAB = ABA[1] + ABA[0] + ABA[1];
    return supernets.indexOf(BAB) != -1;
  });
});
console.log(ipList.length);
