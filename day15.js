...
function calculateAlignmentTime(discs, positions, time) {
  while (!discs.map((disc, i) => {
      return (disc + 1 + i + time) % positions[i];
    }).every(disc => {
      return disc == 0;
    })) {
    time += 1;
...
time = 0;
discs = [1, 10, 2, 1, 3, 5];
positions = [13, 19, 3, 7, 5, 17];
time = calculateAlignmentTime(discs, positions, time);
console.log(time);

// Part 2
discs = [1, 10, 2, 1, 3, 5, 0];
positions = [13, 19, 3, 7, 5, 17, 11];
time = calculateAlignmentTime(discs, positions, time);
console.log(time);
