...
var rooms = input.split('\n').map(room => {
  var divider1 = room.lastIndexOf('-');
  var divider2 = room.lastIndexOf('[');
  var encryptedName = room.slice(0, divider1);
  var sectorID = Number(room.slice(divider1 + 1, divider2));
  var checksum = room.slice(divider2 + 1, -1);

  var sortedLetters = encryptedName.replace(/-/g, "").split("").sort().join("").match(/(\w)\1*/g);
  var x = sortedLetters.sort(function(a, b) {
    if (a.length === b.length) {
      return a.charCodeAt(0) - b.charCodeAt(0);
    }
    return b.length - a.length;
  });
  var real = checksum.split("").every((letter, idx) => letter == sortedLetters[idx][0]) ? true: false;
  return {
    encryptedName,
    sectorID,
    checksum,
...
var realRoomsSectorIDSum = rooms.filter(room => room.real).reduce((sum, room) => sum + room.sectorID, 0);

console.log(realRoomsSectorIDSum);

var decryptedRoomNames = rooms.filter(room => room.real).map(room => {
  var cipher = room.sectorID % 26;
  var decryptedName = room.encryptedName.split("-").map(word => {
    return String.fromCharCode(...word.split("").map(letter => {
      var charCode = letter.charCodeAt(0) + cipher;
      return (charCode > 122) ? charCode - 26 : charCode;
    }));
  }).join(" ");
  return {
    name: decryptedName,
...
console.log(decryptedRoomNames.filter(room => room.name == 'northpole object storage'));
