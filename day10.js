...
var input = `bot 76 gives low to bot 191 and high to bot 21
...
input.split("\n").forEach((instruction) => {
  if (/bot \d+ gives low to (?:output|bot) \d+ and high to (?:output|bot) \d+/g.test(instruction)) {
    var match = /bot (\d+) gives low to (output|bot) (\d+) and high to (output|bot) (\d+)/g.exec(instruction);
    var giverNumber = match[1];
    var takerTypeLow = match[2];
    var takerNumberLow = Number(match[3]);
    var takerTypeHigh = match[4];
    var takerNumberHigh = Number(match[5]);
    if (!bot.hasOwnProperty(giverNumber)) {
      bot[giverNumber] = new Bot(giverNumber);
    }
    if (takerTypeLow == "bot") {
      if (!bot.hasOwnProperty(takerNumberLow)) {
        bot[takerNumberLow] = new Bot(takerNumberLow);
      }
      bot[giverNumber].giveLowTo = bot[takerNumberLow];
    } else {
      if (!output.hasOwnProperty(takerNumberLow)) {
        output[takerNumberLow] = new Output(takerNumberLow);
      }
      bot[giverNumber].giveLowTo = output[takerNumberLow];
    }
    if (takerTypeHigh == "bot") {
      if (!bot.hasOwnProperty(takerNumberHigh)) {
        bot[takerNumberHigh] = new Bot(takerNumberHigh);
      }
      bot[giverNumber].giveHighTo = bot[takerNumberHigh];
    } else {
      if (!output.hasOwnProperty(takerNumberHigh)) {
        output[takerNumberHigh] = new Output(takerNumberHigh);
      }
      bot[giverNumber].giveHighTo = output[takerNumberHigh];
    }

  } else if (/value \d+ goes to (?:output|bot) \d+/g.test(instruction)) {
    var match = /value (\d+) goes to (output|bot) (\d+)/g.exec(instruction);
    var value = Number(match[1]);
    var takerType = match[2];
    var takerNumber = Number(match[3]);
    if (takerType == "bot") {
      if (!bot.hasOwnProperty(takerNumber)) {
        bot[takerNumber] = new Bot(takerNumber);
      }
      bot[takerNumber].takeValue(value);
    } else {
      if (!output.hasOwnProperty(takerNumber)) {
        output[takerNumber] = new Output(takerNumber);
      }
      output[takerNumber].takeValue(value);
    }
  } else {
    console.log("I don't know that instuction");
  }
});

function continueCondition() {
  return Object.keys(bot).some((b) => bot[b].values.length == 2);
}

while (continueCondition()) {
  Object.keys(bot).forEach((b) => {
    if (bot[b].values.length == 2) {
      bot[b].values.sort((a, b) => a - b);
      if (bot[b].values.toString() == "17,61") {
        console.log(bot[b].number + ":" + bot[b].values);
      }
      bot[b].giveLowTo.takeValue(bot[b].values[0]);
      bot[b].giveHighTo.takeValue(bot[b].values[1]);
      bot[b].values = [];
    }
  });
}
...
var outputs = [];
for (var i = 0; i <= 2; i++) {
  outputs.push(output[i].values);
}
console.log(outputs.reduce((a, b) => a * b, 1));
