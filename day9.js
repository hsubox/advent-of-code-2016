...
function decompressedLengthPart1(startIndex, segmentLength) {
  ...
  while (i < startIndex + segmentLength) {
    ...
    if (ch == "(") {
      var skip;
      var subsegmentLength;
      var repeatCount;
      [skip, subsegmentLength] = parseNumber(i, 'x');
      i += skip;
      [skip, repeatCount] = parseNumber(i, ')');
      i += skip;
      outputLength += repeatCount * subsegmentLength;
      i += subsegmentLength;
    } else {
      outputLength += 1;
    }
...
console.log(decompressedLengthPart1(0, input.length));

function decompressedLengthPart2(startIndex, segmentLength) {
  var outputLength = 0;
  var i = startIndex;
  while (i < startIndex + segmentLength) {
    var ch = input[i];
    ...
    if (ch == "(") {
      ...
      [skip, repeatCount] = parseNumber(i, ')');
      i += skip;
      outputLength += repeatCount * decompressedLengthPart2(i, subsegmentLength);
      i += subsegmentLength;
    } else {
      outputLength += 1;
    }
...
console.log(decompressedLengthPart2(0, input.length));
